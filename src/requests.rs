...
mod tests {
    ...
    #[tokio::test]
    async fn run_query() {
        let request = Request::<StatisticsRequest>::from_hex("9295b2323130363131303733303536313037353337ad3632306132363930623165653102a5302e312e300396b232323031323030313038323231383939373907ffffffff").unwrap();
        dbg!(&request);
        let response: messagepack::Response<StatisticsResponse> = api_request(
            &reqwest::Client::new(),
            &(DEFAULT_BASE_URL.to_string() + "/api/statistics/get"),
            request,
        )
        .await
        .unwrap()
        .unwrap();
        dbg!(response.to_hex(), response);
    }
}
